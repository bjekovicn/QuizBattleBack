...
{
    [Authorize]
    public sealed partial class GameHub : Hub<IGameHubClient>
    {
            ...
            if (userId.HasValue)
            {
                ...
                if (!isStillConnected)
                {
                    ...
                    if (roomId.HasValue)
                    {
                        ...
                        // Cancel invites if host of Friend Battle room
                        var room = await _gameService.GetRoomAsync(roomId.Value);
                        if (room.IsSuccess && room.Value.HostPlayerId == userId.Value &&
                            room.Value.GameType == (int)GameType.FriendBattle)
                        {
                            await _inviteService.CancelRoomInvitesAsync(roomId.Value);
                        }
        ...
        public async Task<GamePlayerDto?> JoinRoom(string roomId)
        {
            ...
            {
                await _hubService.NotifyRoomCreatedAsync(roomId, roomResult.Value);
            }

            await _hubService.NotifyPlayerJoinedAsync(roomId, result.Value);
        ...
        public async Task LeaveRoom(string roomId)
        {
            ...
            // Cancel invites if host of Friend Battle room
            var room = await _gameService.GetRoomAsync(Guid.Parse(roomId));
            if (room.IsSuccess && room.Value.HostPlayerId == userId &&
                room.Value.GameType == (int)GameType.FriendBattle)
            {
                await _inviteService.CancelRoomInvitesAsync(Guid.Parse(roomId));
            }
